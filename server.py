...
from apis.base import (
    validate_coordinates, is_coordinate_location, validate_number,
    validate_radius, validate_days, validate_hours, validate_date,
    validate_time, validate_timezone, validate_altitude, validate_unit,
    validate_latitude, validate_longitude, QWEATHER_VALID_DAYS,
    QWEATHER_VALID_HOURS, QWEATHER_VALID_GRID_DAYS, QWEATHER_VALID_GRID_HOURS,
    QWEATHER_VALID_INDICES_DAYS, validate_location_id, validate_country_code,
    LOCATION_ID_DOC, COORDINATES_DOC, ADM_DOC, RANGE_DOC,
)
...
async def city_lookup(location: str, adm: Optional[str] = None, range: Optional[str] = None, number: int = 10, lang: Optional[str] = None) -> dict:
    ...
    Args:
        location: City name (e.g. "Beijing"), LocationID (e.g. "101010100"), or coordinates in longitude,latitude format (e.g. "116.41,39.92"). LocationID is a unique place identifier obtained from this tool.
        adm: Upper administrative district. adm1=province/state (e.g. "Beijing", "Guangdong"), adm2=city (e.g. "Beijing" for districts within it). Note: QWeather's admin levels may not match actual administrative divisions.
        range: Country code (ISO 3166 alpha-2). Example: "cn", "us", "jp"
    ...
    """
    if is_coordinate_location(location):
        validate_coordinates(location)
    number = validate_number(number)
    if range:
        range = validate_country_code(range)
...
async def poi_lookup(location: str, type: str = "scenic", city: str = None, number: int = 10, lang: str = None) -> dict:
    ...
    Args:
        location: Location name (e.g. "Beijing"), LocationID (e.g. "101010100"), or coordinates in longitude,latitude format (e.g. "116.41,39.92"). LocationID is a unique place identifier obtained from city_lookup.
        type: POI type (default: "scenic"). Example: "scenic", "TSTA" (tide station), "ARPT" (airport)
        city: Limit search to specific city by name or LocationID. Example: "Beijing", "101010100"
        number: Number of results (1-20, default 10). Example: 5, 10
        lang: Language. Example: "en", "zh"
...
async def poi_range(location: str, type: str = "scenic", radius: int = 5, number: int = 10, lang: str = None) -> dict:
    ...
    Args:
        location: Coordinates in longitude,latitude format (e.g. "116.41,39.92"). Required for this tool.
...
async def weather_now(location: str, lang: str = None, unit: str = None) -> dict:
    ...
    Args:
        location: LocationID (e.g. "101010100", obtained from city_lookup) or coordinates in longitude,latitude format (e.g. "116.41,39.92"). LocationID is a unique place identifier.
        lang: Language. Example: "en", "zh"
...
async def grid_weather_now(location: str, lang: str = None, unit: str = None) -> dict:
    ...
    Args:
        location: Coordinates in longitude,latitude format (e.g. "116.41,39.92"). Required for grid weather.
        lang: Language. Example: "en", "zh"
        unit: Unit system - "m" for metric, "i" for imperial (default: metric). Example: "m", "i"
...
async def grid_weather_daily(location: str, days: str = "7d", lang: str = None, unit: str = None) -> dict:
    ...
    """
    validate_coordinates(location)
    days = validate_days(days, QWEATHER_VALID_GRID_DAYS)
    if unit:
        unit = validate_unit(unit)
...
async def grid_weather_hourly(location: str, hours: str = "24h", lang: str = None, unit: str = None) -> dict:
    ...
    """
    validate_coordinates(location)
    hours = validate_hours(hours, QWEATHER_VALID_GRID_HOURS)
    if unit:
        unit = validate_unit(unit)
...
async def minutely_precipitation(location: str, lang: str = None) -> dict:
    ...
    Args:
        location: Coordinates in longitude,latitude format (e.g. "116.41,39.92"). China only.
        lang: Language. Example: "en", "zh"
...
async def indices_forecast(location: str, type: str, days: str = "1d", lang: str = None) -> dict:
    ...
    if is_coordinate_location(location):
        validate_coordinates(location)
    days = validate_days(days, QWEATHER_VALID_INDICES_DAYS)
...
async def air_station(location_id: str, lang: str = None) -> dict:
    ...
    Args:
        location_id: Station LocationID (required, obtained from air_now response). Example: "P53763"
        lang: Language. Example: "en", "zh"
...
async def solar_elevation_angle(location: str, date: str, time: str, tz: str, alt: float, lang: str = None) -> dict:
    ...
    Args:
        location: Coordinates in longitude,latitude format (e.g. "116.41,39.92"). Required.
